...
(function () {
    ...
    // =============== 环境判定 ===============
    const isLoginPage = window.location.href.includes('Login/LoginFrameFC.php');
    const isMainPage = window.location.href.includes('ff14bjz.sdo.com/RegionKanTelepo');

    // =============== 统一日志桥接 ===============
    function addLog(message) {
        const timestamp = new Date().toLocaleTimeString('zh-CN', { hour12: false });
        console.log(`[FF14 Helper][${timestamp}] ${message}`);

        // 如果在跨区主页面，输出到日志窗口
        const logWindow = document.getElementById('ff14-log-section');
        if (logWindow) {
    ...
    if (isLoginPage) {
        ...
        const autoAgreeTimer = setInterval(() => {
            if (!GM_getValue('ff14_auto_agree_enabled', true)) return;

            const checkbox = document.getElementById('isAgreementAccept');
            const agreementLabel = document.querySelector('.accept-agreement') || document.querySelector('.agreement-container label');
            ...
            <div id="ff14-control-section">
                ...
                <div class="ff14-config-panel">
                    <div class="ff14-config-row">
                        ...
                        <label for="ff14-opt-agree" style="color:#4CAF50; font-weight:bold;">自动确认用户协议</label>
                    </div>
                    <div class="ff14-config-row">
                        <label>查询间隔(秒):</label>
    ...
    function fetchServerData() {
        addLog('正在请求服务器列表...');
        GM_xmlhttpRequest({
            method: 'GET', url: 'https://ff14bjz.sdo.com/api/orderserivce/queryGroupListTravelSource?appId=100001900',
            onload: (res) => {
    ...
    function showActionArea() {
        ...
        area.innerHTML = `<button id="btn-char" class="ff14-action-btn">选择角色</button><button id="btn-target" class="ff14-action-btn">选择目标大区</button><button id="btn-start" class="ff14-action-btn" disabled>开始尝试</button>`;
    ...
    function updateLinkStartState() { if (selectedRoleInfo && selectedTargetAreaInfo) linkStartBtn.disabled = false; }
    ...
    function toggleMonitoring() {
        if (linkStartBtn.textContent === '取消重试') {
            ...
            addLog('自动重试已取消'); return;
        }
        if (statusCheckTimeoutId) stopMonitoring(); else startMonitoring();
    }

    function startMonitoring() { linkStartBtn.textContent = 'STOP'; linkStartBtn.style.background = '#f44336'; openStatusWindow(); addLog('开启状态查询'); runLoop(); }
    function stopMonitoring() { if (statusCheckTimeoutId) { clearTimeout(statusCheckTimeoutId); statusCheckTimeoutId = null; } linkStartBtn.textContent = '开始重试'; linkStartBtn.style.background = 'linear-gradient(to bottom, #2196F3, #1976D2)'; if (statusWindow && document.body.contains(statusWindow)) document.body.removeChild(statusWindow); addLog('监控已停止'); }

    function runLoop() {
        if (!linkStartBtn || linkStartBtn.textContent !== 'STOP') return;
        GM_xmlhttpRequest({
            method: 'GET', url: 'https://ff14bjz.sdo.com/api/orderserivce/queryGroupListTravelTarget?appId=100001900&areaId=-1&groupId=-1',
            onload: (res) => {
                const data = JSON.parse(res.responseText);
                if (data.return_code === 0 && data.data) {
                    const list = JSON.parse(data.data.groupList);
                    FF14_STATUS_INFO = list.map(a => ({ areaId: a.areaId, areaName: a.areaName, state: a.state, groups: a.groups.map(g => ({ groupId: g.groupId, groupName: g.groupName, queueTime: g.queueTime, groupCode: g.groupCode })) }));
                    updateStatusUI();
                    const target = FF14_STATUS_INFO.find(a => a.areaId == selectedTargetAreaInfo.areaId);
                    if (target && (target.state === 0 || target.state === 1)) {
                        addLog(`目标大区 ${target.areaName} 可用`); stopMonitoring(); playBeep();
                        const avGroups = target.groups.filter(g => g.queueTime === 0);
                        if (avGroups.length > 0) executeMigration(target, avGroups); else addLog('目标虽开放但负载已满');
                    } else {
    ...
    function executeMigration(targetArea, groups) {
        ...
        const params = new URLSearchParams({ appId: '100001900', areaId: sourceArea.areaId, areaName: sourceArea.areaName, groupId: sourceGroup.groupId, groupCode: sourceGroup.groupCode, groupName: sourceGroup.groupName, productId: '1', productNum: '1', migrationType: '4', targetArea: targetArea.areaId, targetAreaName: targetArea.areaName, targetGroupId: targetGroup.groupId, targetGroupCode: targetGroup.groupCode, targetGroupName: targetGroup.groupName, roleList: JSON.stringify(roleList), isMigrationTimes: '0' });
        addLog(`正在提交跨区申请: ${sourceGroup.groupName} -> ${targetGroup.groupName}`);
        GM_xmlhttpRequest({
            method: 'GET', url: `https://ff14bjz.sdo.com/api/orderserivce/travelOrder?${params.toString()}`,
            onload: (res) => {
                const data = JSON.parse(res.responseText);
                if (data.return_code === 0 && data.data?.resultCode === 0) { addLog(`跨区传送申请已提交成功`); showOrderDialog(data.data.orderId); }
                else { addLog(`申请失败: ${data.data?.resultMsg || '接口拒绝'}`); linkStartBtn.textContent = '取消重试'; linkStartBtn.style.background = '#FF9800'; retryTimeoutId = setTimeout(() => { if (linkStartBtn.textContent === '取消重试') startMonitoring(); }, 80000); }
            }
        });
    }
    ...
    function showOrderDialog(id) {
        ...
        document.body.appendChild(div); document.getElementById('btn-go').onclick = () => window.location.href = 'https://ff14bjz.sdo.com/orderList';
    }
    ...
    function openCharWindow() {
        ...
        const aS = characterSelectionWindow.querySelector('#sel-area'), gS = characterSelectionWindow.querySelector('#sel-group');
        ...
        characterSelectionWindow.querySelector('#btn-query').onclick = () => {
            GM_xmlhttpRequest({
                method: 'GET', url: `https://ff14bjz.sdo.com/api/gmallgateway/queryRoleList4Migration?appId=100001900&areaId=${aS.value}&groupId=${gS.value}`,
                onload: (res) => {
                    const data = JSON.parse(res.responseText); const roles = data.data.roleList ? JSON.parse(data.data.roleList) : [];
                    ...
                    roles.forEach(r => {
                        ...
                        li.onclick = () => { list.querySelectorAll('li').forEach(el => el.classList.remove('selected')); li.classList.add('selected'); selectedRoleInfo = { roleId: r.roleId, roleName: r.roleName, selectedAreaId: aS.value, selectedGroupId: gS.value }; characterSelectionWindow.querySelector('#btn-ok').style.display='block'; };
    ...
    function openTargetWindow() {
        ...
        FF14_GROUP_LIST.forEach(a => {
            ...
            li.onclick = () => { ul.querySelectorAll('li').forEach(el => el.classList.remove('selected')); li.classList.add('selected'); selectedTargetAreaInfo = { areaId: a.areaId, areaName: a.areaName }; targetAreaSelectionWindow.querySelector('#btn-tok').style.display='block'; };
            ul.appendChild(li);
        });
        targetAreaSelectionWindow.querySelector('#btn-tok').onclick = () => { addLog(`已选定目标大区: ${selectedTargetAreaInfo.areaName}`); targetAreaSelectionWindow.style.display='none'; updateLinkStartState(); };
        enableDrag(targetAreaSelectionWindow, targetAreaSelectionWindow.querySelector('.ff14-window-header')); targetAreaSelectionWindow.querySelector('.ff14-window-btn').onclick = () => targetAreaSelectionWindow.style.display='none';
    }

    function openStatusWindow() { if (statusWindow && document.body.contains(statusWindow)) return; statusWindow = document.createElement('div'); statusWindow.id = 'ff14-status-window'; statusWindow.innerHTML = `<div class="ff14-window-header"><span>状态实时监控</span></div><div style="display:flex; flex:1; gap:5px; overflow:hidden;"><div id="sa-list" class="status-column"></div><div id="sg-list" class="status-column">详情</div></div>`; document.body.appendChild(statusWindow); enableDrag(statusWindow, statusWindow.querySelector('.ff14-window-header')); }
    function updateStatusUI() { if (!statusWindow) return; const alist = statusWindow.querySelector('#sa-list'); alist.innerHTML = ""; FF14_STATUS_INFO.forEach(a => { const d = document.createElement('div'); d.className = `status-item ${a.state === 0 ? 'status-open' : (a.state === 1 ? 'status-busy' : 'status-blocked')}`; d.textContent = `${a.areaName}`; d.onclick = () => { const gl = statusWindow.querySelector('#sg-list'); gl.innerHTML = ""; a.groups.forEach(g => { const gd = document.createElement('div'); gd.className = `status-item ${g.queueTime === 0 ? 'status-open' : 'status-busy'}`; gd.textContent = `${g.groupName}`; gl.appendChild(gd); }); }; alist.appendChild(d); }); }
    function createWindow(id, title) { const win = document.createElement('div'); win.id = id; win.className = 'ff14-selection-window'; win.innerHTML = `<div class="ff14-window-header"><span>${title}</span><div class="ff14-window-btn">×</div></div>`; return win; }
